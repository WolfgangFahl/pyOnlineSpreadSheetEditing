...
class PropertySelection:
    ...
    def __init__(
        ...
        inputList,
        total: int,
        paretoLevels: typing.Dict[int, Pareto],
        minFrequency: float,
    ):
        """
           Constructor
...
class WikidataItemSearch:
    ...
    def __init__(
        self, app, a1, a2, a3, useComboBox: bool = False, keyStrokeTime: float = 0.65
    ):
        """
        constructor
        ...
        if useComboBox:
            ...
            self.itemInput = self.app.createComboBox(
                labelText="Wikidata item",
                a=a1,
                placeholder="Please type here to search ...",
                value=self.app.itemQid,
                change=self.onItemChange,
            )
        else:
            self.handleSearchResult = self.handleSearchResult4Select
            self.itemInput = self.app.createInput(
                labelText="Wikidata item",
                a=a1,
                placeholder="Please type here to search ...",
                value=self.app.itemQid,
            ...
            self.itemSelect = jp.Select(
                classes="form-select", a=a2, change=self.onItemSelect
            )
    ...
    def updateItemLink(self, wdItem):
        ...
        self.itemLinkDiv.inner_html = Link.create(
            f"{wdItem.url}", itemText, wdItem.description, target="_blank"
        )
...
class WikiDataBrowser(App):
    ...
    def __init__(self, version):
        ...
        self.addMenuLink(
            text="github",
            icon="github",
            href="https://github.com/WolfgangFahl/pyLoDStorage/issues/79",
        )
        self.addMenuLink(
            text="Documentation",
            icon="file-document",
            href="https://wiki.bitplan.com/index.php/Truly_Tabular_RDF",
        )
    ...
    def setEndPoint(self, endpointName: str):
        ...
        self.addMenuLink(
            text="Endpoint", icon="web", href=self.endpointConf.website, target="_blank"
        )
    ...
    async def onChangeEndpoint(self, msg: dict):
        ...
        try:
            ...
            self.showFeedback(
                f"endpoint {self.endpointName} ({self.endpointConf.database}) selected"
            )
        except BaseException as ex:
    ...
    async def onSubclassChange(self, msg):
        ...
        self.subclassPredicate = (
            "wdt:P31/wdt:P279*" if self.withSubclasses else "wdt:P31"
        )
        ...
        try:
            ...
            for key in ["queryf", "queryex"]:
                ...
                tryItUrlEncoded = query.getTryItUrl(
                    baseurl=self.endpointConf.website,
                    database=self.endpointConf.database,
                )
                tryItLink = Link.create(
                    url=tryItUrlEncoded,
                    text="try it!",
                    tooltip=f"try out with {self.endpointConf.name}",
                    target="_blank",
                )
    ...
    async def trulyTabularAnalysis(self, tt):
        ...
        if getattr(self, "propertySelection", None) is None:
            self.showFeedback(
                "no property Selection available for truly tabular analysis"
            )
            await self.wp.update()
            return
        ...
        for _i, (propertyId, propRecord) in enumerate(selectedItems):
            ...
            future = executor.submit(
                self.wikiTrulyTabularPropertyStatsAndUpdateTable, tt, propertyId
            )
        ...
        while len(analysisTasks) > 0:
            ...
            self.showFeedback(
                f"{completedTasks }/{propertyCount}: querying statistics - (completed statistics for {props})..."
            )
    ...
    async def getMostFrequentlyUsedProperties(self, tt: TrulyTabular):
        ...
        if self.propertyQueryDisplay is None:
            self.propertyQueryDisplay = self.createQueryDisplay(
                "property Query", a=self.colA4, wdItem=tt.item
            )
    ...
    def addSelectionColumn(
        self, table: jp.Table, column: str, checkedCondition: callable, onInput=None
    ):
        """
    ...
    async def getCountQuery(self, tt: TrulyTabular):
        ...
        if self.countQueryDisplay is None:
            self.countQueryDisplay = self.createQueryDisplay(
                "count Query", a=self.colA4, wdItem=tt.item
            )
    ...
    async def getPropertiesTable(self, tt: TrulyTabular, ttquery: Query):
        ...
        """
        self.showFeedback(
            f"running query for most frequently used properties of {str(self.tt)} ..."
        )
        await self.wp.update()
        if self.debug:
        ...
        self.addSelectionColumn(
            self.ttTable,
            "label",
            lambda record: record["type"] == "WikibaseItem"
            and self.propertySelection.hasMinFrequency(record),
        )
        self.addSelectionColumn(
            self.ttTable,
            "select",
            lambda record: self.propertySelection.hasMinFrequency(record)
            and record["propertyId"] != "P31",
        )
    ...
    async def selectItem(self, itemId: typing.Union[str, None]):
        ...
        try:
            ...
            await self.getMostFrequentlyUsedProperties(
                self.tt
            ),  # uses: ttcount, |defines: ttquery,propertyQueryDisplay
            await self.getPropertiesTable(
                self.tt, self.ttquery
            ),  # uses: ttquery  | defines: propertyList, propertySelection, ttTable
    ...
    async def onParetoSelect(self, msg):
        ...
        try:
            ...
            self.showFeedback(
                f"pareto level {self.paretoLevel} (>={minPropertyFrequencyStr}% )selected"
            )
            if self.generateQueryButton is not None:
    ...
    def createParetoSelect(self, a, ai):
        ...
        Returns:
            jp.Select
        """
        pselect = self.createSelect(
            "Pareto", str(self.paretoLevel), change=self.onParetoSelect, a=a
        )
...
"""
    ...
    async def itemsearch(self):
        ...
        self.wdItemSearch = WikidataItemSearch(
            self, self.colA1, self.colA2, self.colB1, useComboBox=True
        )
        return self.wp
    ...
    async def settings(self):
        ...
        self.languageSelect = self.createSelect(
            "Language", "en", a=self.colC1, change=self.onChangeLanguage
        )
        ...
        self.endpointSelect = self.createSelect(
            "Endpoint", self.endpointName, a=self.colC1, change=self.onChangeEndpoint
        )
    ...
    async def content(self) -> jp.WebPage:
        ...
        self.setupRowsAndCols()
        self.wdItemSearch = WikidataItemSearch(
            self, a1=self.colA1, a2=self.colA2, a3=self.colB1, useComboBox=True
        )
        self.subclassCheckbox = self.createCheckbox(
            "subclasses",
            a=self.colA2,
            checked=self.withSubclasses,
            input=self.onSubclassChange,
        )
