...
from fb4.widgets import ButtonField, Link
...
class PropertySelection:
    ...
    def prepare(
        ...
        for i, prop in enumerate(self.propertyList):
            ...
            prop[
                "select"
            ] = f'<input name="{checkBoxName}" value="{itemId}" id="{itemId}" type="checkbox"{checked}>'
        pass
...
class TrulyTabularForm(FlaskForm):
    ...
    def setPropertyList(self, propertyList: list, total: int, paretoList: list):
        ...
        propertySelection.prepare(
            propertyList, total, paretoList, checkBoxName=self.checkBoxName
        )
