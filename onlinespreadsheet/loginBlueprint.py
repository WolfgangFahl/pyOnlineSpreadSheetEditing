...
class LoginBluePrint(object):
    ...
    def getAllUsers(self):
        ...
        return self.appWrap.render_template(
            "users.html", "users", "users", users=usersTable
        )
    ...
    def editUser(self, userId):
        ...
        return self.appWrap.render_template(
            "userForm.html", "editUser", "editUser", formTitle="Edit User", form=form
        )
...
class UserManager(EntityManager):
    ...
    def __init__(self, storageConfig: StorageConfig = None):
        ...
        if not self.isCached():
            ...
            self.initSQLDB(
                self.getSQLDB(self.getCacheFile()), withDrop=False, withCreate=True
            )
    ...
    def updateUser(self, user: User):
        ...
        db.c.execute(
            f"UPDATE {self.tableName} SET {vars} WHERE id == ?", (*params, user.id)
        )
        db.c.commit()
...
class UserForm(FlaskForm):
    ...
    wikidataid = StringField(
        ...
        [
            validators.Regexp(
                "Q[1-9]\d*",
                message="Must be a valid  Wikidata Q identifier (Q43649390) ",
            )
        ],
    )
    roles = SelectMultipleField(
        ...
        coerce=Roles.coerce,
        render_kw={"class_": ""},
    )  # ToDo: Change to ListField and checkboxes
